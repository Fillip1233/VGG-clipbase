...
def parser_args():
    ...
    parser.add_argument('--dataset_dir', help='dir of dataset', default='/mnt/cephfs/dataset/zhenjie/')
...
def main():
    ...
    args.dist_url='tcp://127.0.0.1:3455'
...
def main_worker(args, logger):
    ...
    #     gamma_neg=args.gamma_neg, gamma_pos=args.gamma_pos,  #gamma_neg负样本调节参数，gamma_pos正样本调节参数
    #     disable_torch_grad_focal_loss=True,
    #     eps=args.eps,
    # )                  #不对称的损失函数，它可以根据正负样本之间的不平衡性对误差进行加权
    ...
    _, mAP = validate(train_loader, model, criterion, args, logger)
...
def validate(val_loader, model, criterion, args, logger):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):    #val_loader测试集，这里改target
            ...
            # save some data
            _item = torch.cat((output_sm.detach().cpu(), target.detach().cpu()), 1)  #_item[16,70],output_sm[16,70]
            saved_data.append(_item)   #saved_data[53280, 70]
