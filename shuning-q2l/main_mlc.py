...
import _init_paths
from dataset.get_dataset import get_datasets  #baoliu
from dataloader.action_genome import AG, cuda_collate_fn   #AG数据集
...
def main_worker(args, logger):
    ...
    if args.evaluate:
        ...
        # _, mAP = validate1(train_loader, model, criterion, args, logger)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0:
            ...
            if dist.get_rank() == 0:
                ...
                }, is_best=is_best, epoch=epoch+1,filename=os.path.join(args.output, 'checkpoint.pth.tar'))
            ...
            if math.isnan(loss) or math.isnan(loss_ema):
                save_checkpoint({
                    ...
                    'best_mAP': best_mAP,
                    'optimizer' : optimizer.state_dict(),
                }, is_best=is_best,epoch=epoch+1,filename=os.path.join(args.output, 'checkpoint_nan.pth.tar'))
...
def train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger):
    ...
    for i, (images, target, _) in enumerate(train_loader):
        ...
        # images = images.reshape(8*args.batch_size,360,640)
        images = images.reshape(8*args.batch_size,3,448,448)
        # images = images.reshape(4*2,3,448,448)
        images = images.cuda(non_blocking=True)
        # images = [img.cuda(non_blocking=True) for img in images]
        target = target.reshape(8*args.batch_size,35)
        # target = target.reshape(4*2,36)
        target = target.cuda(non_blocking=True)
        
        t1=data[2].cuda(non_blocking=True)
        # index_list = [4249, 3709, 4838]
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            #     if index in t1:
            #         inde = torch.where(t1 == index)[0].cpu()
            #         obj_gt_1 = data[1].cpu()
            #         oo = obj_gt_1[inde][0, :, :]
            #         np.savetxt(f"output_{index}.txt", oo, fmt='%.4f')
        ...
        # for index in index_list:
        #     if index in t1:
        #         inde1= torch.where(t1==index)[0].cpu()
        #         s1=output.cpu()
        #         s1=s1[inde1*8:inde1*8+8,:]
        #         filenamere = f"output1{epoch}_{index}.txt"
...
# @torch.no_grad()
# def validate(train_loader, model, criterion, args, logger):
#     batch_time = AverageMeter('Time', ':5.3f')
#     losses = AverageMeter('Loss', ':5.3f')
#     mem = AverageMeter('Mem', ':.0f', val_only=True)
...
#             target = target.reshape(8*args.batch_size,36)
#             images = images.reshape(8*args.batch_size,3,448,448)
#             images = images.cuda(non_blocking=True)
#             target = target.cuda(non_blocking=True)
...
#             if i % args.print_freq == 0 and dist.get_rank() == 0:
#                 # progress.display(len(val_loader) + i, logger)
#                 progress.display(i, logger)
...
#         if dist.get_rank() == 0:
#             print("Calculating trainmAP:")
#             filenamelist = ['saved_data_tmp.{}.txt'.format(ii) for ii in range(dist.get_world_size())]
#             metric_func = voc_mAP                
#             mAPtrain, aps = metric_func([os.path.join(args.output, _filename) for _filename in filenamelist], args.num_class, return_each=True)
            
#             logger.info("  mAP: {}".format(mAPtrain))
#             logger.info("   aps: {}".format(np.array2string(aps, precision=5)))
#         else:
#             mAPtrain = 0
...
def save_checkpoint(state, is_best,epoch,filename='checkpoint.pth.tar'):
    ...
    # else :
        torch.save(state,os.path.split(filename)[0]+'/model_{}.pth.tar'.format(epoch))
