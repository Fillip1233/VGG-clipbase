...
def voc_mAP(imagessetfilelist, num, return_each=False):
    ...
    seg = np.array([x.strip().split(' ') for x in lines]).astype(float)   #[13056,70]
    gt_label = seg[:,num:].astype(np.int32)   #[13056,70]
    ...
    for class_id in range(class_num):     #第0类
        confidence = seg[:,class_id]     #(13056,)
    ...
    # aps_filtered = np.array(aps)[np.array(aps) != 0]
    # 获取不包含nan值的子数组
    aps_filtered = np.array(aps)[~np.isnan(aps)]
    # 计算平均值
    mAP = np.mean(aps_filtered)
    if return_each:
        return mAP, aps
    return mAP
