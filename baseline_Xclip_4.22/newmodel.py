...
# class FC(nn.Module):
#     def __init__(self, in_features, out_features, num_layers=3, hidden_dim=256):
#         super(FC, self).__init__()
        
#         self.layers = nn.ModuleList()
#         self.layers.append(nn.Linear(in_features, hidden_dim, bias=False))
#         if num_layers > 2:
...
class base_Model(nn.Module):
    def __init__(self):
        ...
        # self.visual_fc = FC(768, 512)
        # self.text_fc = FC(512, 512)
...
#logits_per_text[i][logits_per_text[i] != max_values[i]] = 0 