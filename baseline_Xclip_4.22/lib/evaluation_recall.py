...
#     Pd_data.to_csv(path,encoding='utf-8')
...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph(self, gt, pred,video_figure):
        ...
        for idx, frame_gt in enumerate(gt):            
            ...
            #len(frame_gt)看有几个object，然后把第一个设为person
            ...
            #对一个帧里不同pair的关系和在一起，按（h-o-r,o-h-r,h-o-r)顺序排布
            gt_entry = {
            ...
            evaluate_from_dict(gt_entry, pred_entry, self.mode, self.result_dict,
                               iou_thresh=self.iou_threshold, method=self.constraint, threshold=self.semithreshold,videoidx=video_figure,frameidx=idx)

def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9,videoidx=None,frameidx=None,**kwargs):
    """
    ...
    pred_to_gt, pred_5ples, rel_scores = evaluate_recall(
                ...
                predicate_scores, obj_scores, phrdet= mode=='phrdet',methodname=method,videoidx=videoidx,frameidx=frameidx,
                **kwargs)
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
                    ...
                    iou_thresh=0.5, phrdet=False,methodname=None,videoidx=None,frameidx=None):
    """
    ...
    fpgt=open('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/frame{}/gt.npy'.format(videoidx,frameidx),"a")
    # fpgt.writelines("gt_triplets:\n")
    # fpgt.writelines(str(gt_triplets.tolist()))
    # fpgt.writelines("\ngt_triplet_boxes:\n")
    # fpgt.writelines(str(gt_triplet_boxes.tolist()))
    # fpgt.close()
    savegtdata={'triples':gt_triplets,'boxes':gt_triplet_boxes}
    # savetocsv(savegtdata,'/mnt/cephfs/home/alvin/yingqi/STTran/video.sgdet/video{}/frame{}/gt.csv')
    np.save('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/frame{}/gt.npy'.format(videoidx,frameidx),savegtdata,allow_pickle=True)
    ...
    savepreddata={'triples':pred_triplets,'boxes':pred_triplet_boxes,'rlsscore':relation_scores}
    if methodname == 'semi':
        # fp=open('/mnt/cephfs/home/alvin/yingqi/STTran/vpredcls2/video{}/frame{}/smi.npy'.format(videoidx,frameidx),"a")
        np.save('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/frame{}/semi.npy'.format(videoidx,frameidx),savepreddata,allow_pickle=True)
        # fp.writelines("pre_triplets:\n")
        # fp.writelines(str(pred_triplets.tolist()))
        # fp.writelines("\npred_triplet_boxes:\n")
        # fp.writelines(str(pred_triplet_boxes.tolist()))
        # fp.writelines("\nrelation_scores:\n")
        # fp.writelines(str(relation_scores.tolist()))
        # fp.close()
    elif methodname == 'no':
        # fp=open('/mnt/cephfs/home/alvin/yingqi/STTran/vpredcls2/video{}/frame{}/no.npy'.format(videoidx,frameidx),"a")
        np.save('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/frame{}/no.npy'.format(videoidx,frameidx),savepreddata,allow_pickle=True)
        # fp.writelines("pre_triplets:\n")
        # fp.writelines(str(pred_triplets.tolist()))
        # fp.writelines("\npred_triplet_boxes:\n")
        # fp.writelines(str(pred_triplet_boxes.tolist()))
        ...
        # fp.writelines(str(relation_scores.tolist()))
        # fp.close()
    elif methodname == 'with':
        # fp=open('/mnt/cephfs/home/alvin/yingqi/STTran/vpredcls2/video{}/frame{}/with.npy'.format(videoidx,frameidx),"a")
        np.save('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/frame{}/with.npy'.format(videoidx,frameidx),savepreddata,allow_pickle=True)
        # fp.writelines("pre_triplets:\n")
        # fp.writelines(str(pred_triplets.tolist()))
        # fp.writelines("\npred_triplet_boxes:\n")
        # fp.writelines(str(pred_triplet_boxes.tolist()))
